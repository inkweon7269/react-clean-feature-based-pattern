...
- 프로젝트 아키텍처 규칙(CLAUDE.md, docs/GUIDE.md)을 기준으로 코멘트의 타당성을 판단
- 코드 수정 시 typecheck/build/test 검증을 포함하여 안전한 반영을 보장

### 1.3 제약사항

- Claude Code MAX 요금제를 사용하므로 GitHub Actions + API Key 방식(별도 과금)은 제외
- 로컬에서 `/respond-coderabbit`로 수동 실행하는 방식으로 진행
- e2e 테스트는 NestJS 백엔드(`localhost:3000`) 의존이므로 검증 단계에서 선택적으로만 실행
...
3. CLAUDE.md / docs/GUIDE.md의 아키텍처 규칙과 기존 패턴을 참조하여 타당성을 판단
...
PR: #42 — feat(posts): add domain and infrastructure
처리 대상: 5개 코멘트 (전체 8개 중 3개 이미 처리됨)

| # | 파일                                         | 라인 | 분류    | 요약                                  |
|---|----------------------------------------------|------|---------|---------------------------------------|
| 1 | src/infrastructure/api/posts/PostsApiRepository.ts | 18 | ACCEPT  | params undefined 가드 추가             |
| 2 | src/features/posts-create/useCreatePost.ts   | 12   | REJECT  | UseCase 분리 — 패스스루는 미생성 정책   |
| 3 | src/features/posts-list/PostsList.tsx        | 45   | PARTIAL | 키 prop 추가, useMemo 제안은 미반영     |
| 4 | src/domain/posts/entities.ts                 | 30   | SKIP    | 명명 의도 질문에 대한 답변              |
| 5 | src/test/mocks/handlers.ts                   | 88   | ACCEPT  | UUID 정규식 anchor 보강                |
...
**규칙:**
- CLAUDE.md / docs/GUIDE.md의 아키텍처 규칙을 준수하여 수정
- 기존 코드 패턴과 일관성 유지 (auth 도메인이 좋은 참조점)
...
푸시 정책:
- **현재 브랜치가 보호 브랜치(`main`/`dev`)인 경우** — 커밋만 하고 푸시는 사용자에게 안내
- **그 외 작업 브랜치(`feat/*`, `fix/*` 등)** — `git push` 실행
...
| # | 파일 | 작업 |
|---|------|------|
| 1 | `.claude/skills/respond-coderabbit/SKILL.md` | 스킬 정의 |
| 2 | `docs/coderabbit-review-automation.md` | 본 문서 (자동화 기획) |

권한 요구사항 (`.claude/settings.local.json`):
- `Bash(gh api:*)`, `Bash(gh pr:*)`, `Bash(gh repo:*)` — gh CLI
- `Bash(git *)` — 이미 허용됨

권한이 없으면 호출 시점에 사용자가 승인하는 흐름으로 동작한다.
